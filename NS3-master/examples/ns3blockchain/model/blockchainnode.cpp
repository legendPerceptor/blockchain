...
namespace ns3::blockchain{
    ...
    TypeId BlockchainNode::GetTypeId() {
        static TypeId tid = TypeId("ns3::blockchain::BlockchainNode")
    ...
    BlockchainNode::BlockchainNode():m_isMiner(false), m_averageTransacionSize(522.4),
    m_transactionIndexSize(2),m_blockchainPort(8333),m_secondsPerMin(60),
    m_countBytes(4),m_blockchainMessageHeader(90),m_inventorySizeBytes(36),
    m_getHeaderSizeBytes(72),m_headersSizeBytes(81), m_blockHeadersSizeBytes(81){
    ...
    Ptr<Socket> BlockchainNode::GetListeningSocket() const {
        NS_LOG_FUNCTION(this);
        return m_socket;
    }

    std::vector<Ipv4Address> BlockchainNode::GetPeerAddress() const {
        NS_LOG_FUNCTION(this);
    ...
    void BlockchainNode::SetPeersDownloadSpeeds(const std::map<Ipv4Address, double>& peerDownloadSpeeds) {
        NS_LOG_FUNCTION(this);
        m_peersDownloadSpeeds = peerDownloadSpeeds;
    }

    void BlockchainNode::SetPeersUploadSpeeds(const std::map<Ipv4Address, double>& peerUploadSpeeds) {
        NS_LOG_FUNCTION(this);
        m_peersUploadSpeeds = m_peersUploadSpeeds;
    }

    void BlockchainNode::SetNodeInternetSpeeds(const nodeInternetSpeed& internetSpeed) {
        NS_LOG_FUNCTION(this);
        m_downloadSpeed = internetSpeed.downloadSpeed*1000000/8;
        m_uploadSpeed = internetSpeed.uploadSpeed*1000000/8;
    }

    void BlockchainNode::SetNodeStats(std::unique_ptr<nodeStatistics> nodeStats) {
        NS_LOG_FUNCTION(this);
        m_nodeStats = std::move(nodeStats);
    }

    void BlockchainNode::SetProtocolType(ProtocolType protocolType) {
        NS_LOG_FUNCTION(this);
        m_protocolType = protocolType;
    }

    void BlockchainNode::SetCommitterType(CommitterType cType) {
        NS_LOG_FUNCTION(this);
    ...
    void BlockchainNode::StartApplication() {
        ...
        std::default_random_engine eng(seed + GetNode()->GetId());
        ...
        if(m_committerType == CommitterType::COMMITTER)
        {
            m_nodeStats->nodeType = 0;
        }
        else if(m_committerType == CommitterType::ENDORSER)
        {
            m_nodeStats->nodeType = 1;
        }
        else if(m_committerType == CommitterType ::CLIENT)
        {
            ...
            if(InetSocketAddress::IsMatchingType(from)){
                ...
                while((pos = totalReceivedData.find(delimiter)) != std::string::npos) {
                    ...
                    NS_LOG_INFO("At time " << Simulator::Now().GetSeconds()
                                           << "s Blockchain node " << GetNode()->GetId() << " received from "
                                           << InetSocketAddress::ConvertFrom(from).GetIpv4()
                                           << " port " << InetSocketAddress::ConvertFrom(from).GetPort()
                                           << " with info = " << buffer.GetString());

                    switch(static_cast<Messages>(d["message"].GetInt())) {
                        case Messages::INV: {
                            NS_LOG_INFO("INV");

                            if (m_committerType != CommitterType::CLIENT) {
                                unsigned int j;
                                std::vector<std::string> requestBlocks;
                                std::vector<std::string>::iterator block_it;

                                m_nodeStats->invReceivedBytes += m_blockchainMessageHeader
                                                                 + m_countBytes +
                                                                 d["inv"].Size() * m_inventorySizeBytes;
                                ...
                                for (j = 0; j < d["inv"].Size(); j++) {
                                    ...
                                    int height = std::stoi(parsedInv.substr(0, invPos));
                                    int minerId = std::stoi(parsedInv.substr(invPos + 1, parsedInv.size()));

                                    if (m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId)
                                        || ReceivedButNotValidated(parsedInv)) {
                                        ...
                                        if (m_invTimeouts.find(parsedInv) == m_invTimeouts.end()) {
                                            ...
                                            timeout = Simulator::Schedule(m_invTimeoutMinutes,
                                                                          &BlockchainNode::InvTimeoutExpired, this,
                                                                          parsedInv);
                                ...
                                if (!requestBlocks.empty()) {
                                    ...
                                    SendMessage(Messages::INV, Messages::GET_HEADERS, d, from);
                                    SendMessage(Messages::INV, Messages::GET_DATA, d, from);
                                }
                            }
                            break;
                        }

                        case Messages::REQUEST_TRANS: {
                            ...
                            if (m_committerType != CommitterType::CLIENT) {
                                ...
                                m_nodeStats->getDataReceivedBytes += m_blockchainMessageHeader +
                                                                     m_countBytes +
                                                                     d["transactions"].Size() * m_inventorySizeBytes;
                                ...
                                for (j = 0; j < d["transactions"].Size(); j++) {
                                    ...
                                    if (HasTransaction(nodeId, transId)) {
                                        ...
                                        if (m_committerType == CommitterType::ENDORSER) {
                                            newTrans.SetExecution(GetNode()->GetId());
                                            m_totalEndorsement++;
                                            m_meanEndorsementTime = m_meanEndorsementTime *
                                                                    static_cast<double>(m_totalEndorsement - 1) +
                                                                    (Simulator::Now().GetSeconds() - timestamp) /
                                                                    static_cast<double>(m_totalEndorsement);
                                            ExecuteTransaction(newTrans,
                                                               InetSocketAddress::ConvertFrom(from).GetIpv4());
                                        } else {
                                            AdvertiseNewTransaction(newTrans, Messages::REQUEST_TRANS,
                                                                    InetSocketAddress::ConvertFrom(from).GetIpv4());
                                        }
                                    }
                                }
                        ...
                        case Messages::REPLY_TRANS: {
                            NS_LOG_INFO("REPLY_TRANS");

                            unsigned int j;
                            std::vector<Transaction>::iterator trans_it;

                            m_nodeStats->getDataReceivedBytes += m_blockchainMessageHeader + m_countBytes +
                                    d["transactions"].Size() * m_inventorySizeBytes;
                            ...
                            for(j=0; j<d["transactions"].Size(); j++) {
                                ...
                                if(HasReplyTransaction(nodeId, transId, transExecution)) {
                                    ...
                                    <<" and transId = " << transId);
                                } else if(GetNode()->GetId() != nodeId) {
                                    // the node is committer
                                    Transaction newTrans(nodeId, transId, timestamp);
                                    ...
                                    AdvertiseNewTransaction(newTrans, Messages::REPLY_TRANS, InetSocketAddress::ConvertFrom(from).GetIpv4());
                                    ...
                                    for(it_tran = m_waitingEndorsers.begin();it_tran < m_waitingEndorsers.end(); it_tran++) {
                                        if(it_tran->GetTransNodeId() == nodeId && it_tran->GetTransId()==transId
                                        && it_tran->GetExecution()==transExecution) {
                                            NS_LOG_INFO("REPLY_TRANS: Blockchain node "<< GetNode()->GetId()
                                    ...
                                    if(m_waitingEndorsers.size() == m_numberofEndorsers) {
                                        AdvertiseNewTransaction(newTrans, Messages::MSG_TRANS, InetSocketAddress::ConvertFrom(from).GetIpv4());
                            ...
                            if(m_committerType != CommitterType::CLIENT) {
                                ...
                                for(j = 0; j < d["transactions"].Size(); j++) {
                                    ...
                                    if(HasMessageTransaction(nodeId, transId)) {
                                        ...
                                        << " has transaction which is already executed and not validated ndoeID= "
                                        <<  nodeId <<" and transId = " << transId);
                                    } else {
                                        Transaction newTrans(nodeId, transId, timestamp);
                                        newTrans.SetExecution(transExecution);
                        ...
                        case Messages::RESULT_TRANS: {
                            ...
                            m_nodeStats->getDataReceivedBytes += m_blockchainMessageHeader
                                    + m_countBytes + d["transactions"].Size()*m_inventorySizeBytes;
                            ...
                            for(j=0; j<d["transactions"].Size(); j++) {
                                ...
                                if(HasResultTransaction(nodeId, transId)) {
                                    ...
                                    if(GetNode()->GetId() != nodeId) {
                                        AdvertiseNewTransaction(newTrans, Messages::RESULT_TRANS, InetSocketAddress::ConvertFrom(from).GetIpv4());
                                    }else {
                                        m_totalCreatedTransaction++;
                                        m_meanLatency = (m_meanLatency* static_cast<double>(m_totalCreatedTransaction - 1) +
                                                (Simulator::Now().GetSeconds() - timestamp)) / static_cast<double>(m_totalCreatedTransaction);
                                    }
                                }
                            }
                        ...
                        case Messages::GET_HEADERS: {
                            ...
                            if(m_committerType != CommitterType::CLIENT) {
                                ...
                                for(j = 0; j<d["blocks"].Size(); j++) {
                                    ...
                                    int height = std::stoi(blockHash.substr(0, invPos));
                                    int minerId = std::stoi(blockHash.substr(invPos+1,blockHash.size()));
                                    ...
                                    if(m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId)) {
                                        ...
                                        requestHeaders.push_back(std::move(newBlock));
                                ...
                                if(!requestHeaders.empty()) {
                                    ...
                                    d.AddMember("blocks", array, d.GetAllocator());
                                    SendMessage(Messages::GET_HEADERS, Messages::HEADERS,d, from);
                                }
                            }
                            break;
                        }
                        case Messages::HEADERS: {
                            ...
                            if(m_committerType != CommitterType::CLIENT) {
                                ...
                                m_nodeStats->headersReceivedBytes += m_blockchainMessageHeader + m_countBytes
                                        +d["blocks"].Size()*m_headersSizeBytes;
                                ...
                                for(j=0; j<d["blocks"].Size();j++) {
                                    ...
                                    m_onlyHeadersReceived[blockHash] = newBlockHeaders;
                                    stringStream.clear();
                                    stringStream.str("");
                                    ...
                                    if(!m_blockchain.HasBlock(parentHeight, parentMinerId) && !m_blockchain.IsOrphan(parentHeight, parentMinerId)
                                       && !ReceivedButNotValidated(parentBlockHash)) {
                                        ...
                                        if(m_invTimeouts.find(parentBlockHash) == m_invTimeouts.end())
                                        {
                                            ...
                                            {
                                                requestHeaders.emplace_back(parentBlockHash);
                                            }
                                ...
                                if(!requestHeaders.empty()) {
                                    ...
                                    d.AddMember("blocks", array, d.GetAllocator());
                                    SendMessage(Messages::HEADERS, Messages::GET_HEADERS, d, from);
                                    SendMessage(Messages::HEADERS, Messages::GET_DATA, d, from);
                                }
                        ...
                        case Messages::GET_DATA: {
                            ...
                            if(m_committerType!= CommitterType::CLIENT) {
                                ...
                                m_nodeStats->getDataReceivedBytes = m_blockchainMessageHeader + m_countBytes +
                                        d["blocks"].Size() * m_inventorySizeBytes;
                                ...
                                for(j=0; j<d["blocks"].Size(); j++) {
                                    ...
                                    if(m_blockchain.HasBlock(height, minerId)) {
                                        NS_LOG_INFO("GET_DATA: Blockchain node "<< GetNode()->GetId()
                                            << " has the block with height = "<<height<<" and minerId = "<<minerId);
                                        Block newBlock(m_blockchain.ReturnBlock(height, minerId));
                        ...
                        case Messages::BLOCK: {
                            ...
                            if(m_committerType != CommitterType::CLIENT) {
                                ...
                                if(m_receiveBlockTimes.empty() || Simulator::Now().GetSeconds() > m_receiveBlockTimes.back())
                                {
                        ...
                        default: {
                            NS_LOG_INFO("Messages Type fall into default");
                            break;
                        }
                    }
                    totalReceivedData.erase(0, pos + delimiter.length());
                ...
                //delete[] packetInfo;  potentially will cause memory leaks  --- use smart pointer
            }else {
                NS_LOG_INFO("At time " << Simulator::Now().GetSeconds()
    ...
    void BlockchainNode::HandleAccept(Ptr<Socket> socket, const Address &from) {
        NS_LOG_FUNCTION(this);
        socket->SetRecvCallback (MakeCallback(&BlockchainNode::HandleRead, this));
    }

    void BlockchainNode::HandlePeerClose(Ptr<Socket> socket) {
        NS_LOG_FUNCTION(this);
    }
    ...
    void BlockchainNode::ReceivedBlockMessage(std::string &blockInfo, Address &from) {
        NS_LOG_FUNCTION(this);
        ...
        for(unsigned int j=0; j<d["blocks"].Size();j++) {
            ...
            ostringstream << height << "/" <<minerId;
            blockHash = ostringstream.str();
            ...
            if(!m_blockchain.HasBlock(parentHeight, parentMinerId) && !m_blockchain.IsOrphan(parentHeight, parentMinerId)
                ...
                for(unsigned int i =0; i<d["blocks"][j]["transactions"].Size(); i++) {
                    ...
                    newTransactions.emplace_back(transNodeId, transId, timestamp);
                }
    ...
    void BlockchainNode::ReceiveBlock(const Block &block) {
        NS_LOG_FUNCTION(this);
        ...
        stringStream << block.GetBlockHeight() << "/" << block.GetMinerId();
        blockHash = stringStream.str();

        if(m_blockchain.HasBlock(block) || m_blockchain.IsOrphan(block) || ReceivedButNotValidated(blockHash)){
            NS_LOG_INFO("ReceivedBlock: Blockchain node "<<GetNode()->GetId()
            ...
            m_receivedNotValidated[blockHash] = block;
    ...
    void BlockchainNode::SendBlock(const std::string& packetInfo, Address &from) {
        NS_LOG_FUNCTION(this);

        NS_LOG_INFO("SendBlock: At time "<<Simulator::Now().GetSeconds()
        <<"s blockchain node "<< GetNode()->GetId() << " sent "<< packetInfo<< " to " <<InetSocketAddress::ConvertFrom(from).GetIpv4());

        SendMessage(Messages::GET_DATA, Messages::BLOCK, packetInfo, from);

    }

    void BlockchainNode::ReceivedHigherBlock(const Block &newBlock) {
        NS_LOG_FUNCTION(this);
        NS_LOG_INFO("ReceivedHigherBlock: blockchain node : " <<GetNode()->GetId()
                                                              << " added a new block in the m_blockchain with higher height");
    }

    void BlockchainNode::ValidateBlock(const Block &newBlock) {
        //假装验证区块
        NS_LOG_FUNCTION(this);
        auto parent = m_blockchain.GetParent(newBlock);
        if(parent) {
            const int averageBlockSizeBytes = 238263;   // 人工设定，需要调参
    ...
    void BlockchainNode::ValidateTransaction(const Block &newBlock) {
        std::vector<Transaction>                requestTransactions;
        std::vector<Transaction>::iterator      trans_it;
    ...
    void BlockchainNode::AfterBlockValidation(const Block &newBlock) {
        NS_LOG_FUNCTION(this);
    ...
    void BlockchainNode::ValidateOrphanChildren(const Block &newBlock) {
        NS_LOG_FUNCTION(this);

        auto children = m_blockchain.GetOrphanChildrenPointers(newBlock).value();
        if(children.empty()) {
            NS_LOG_INFO("ValidateOrphanChildren: Block has no orphan children\n");
        }else {
            NS_LOG_INFO("ValidateOrphanChildren: Block has orphan children:");
            for(auto block_it = children.begin(); block_it<children.end();block_it++) {
    ...
    void BlockchainNode::AdvertiseNewBlock(const Block &newBlock) {
        ...
        if(m_protocolType==ProtocolType::STANDARD_PROTOCOL) {
            value = static_cast<int>(Messages::INV);
        ...
        for(auto i=m_peersAddresses.begin();i!=m_peersAddresses.end();i++) {
    ...
    void BlockchainNode::AdvertiseNewTransaction(const Transaction &newTrans, Messages megType,
                                                 Ipv4Address receivedFromIpv4) {
        NS_LOG_FUNCTION(this);
    ...
    void BlockchainNode::CreateTransaction() {
        ...
        value = static_cast<int>(Messages::REQUEST_TRANS);
        transD.AddMember("message", value, transD.GetAllocator());
    ...
    void BlockchainNode::ScheduleNextTransaction() {
        ...
        std::uniform_real_distribution<double> distreal(1,m_creatingTransactionTime);
        double tTime = distreal(eng);
        m_nextTransaction = Simulator::Schedule(Seconds(tTime), &BlockchainNode::CreateTransaction, this);
    }

    void BlockchainNode::ExecuteTransaction(const Transaction &newTrans, Ipv4Address receivedFromIpv4) {
        NS_LOG_FUNCTION(this);
        ...
        value = static_cast<int>(Messages::REPLY_TRANS);
        transD.AddMember("message", value, transD.GetAllocator());
    ...
    void BlockchainNode::NotifyTransaction(const Transaction &newTrans) {
        NS_LOG_FUNCTION(this);
        ...
        value = static_cast<int>(Messages::RESULT_TRANS);
        transD.AddMember("message", value, transD.GetAllocator());
    ...
    void BlockchainNode::SendMessage(Messages receivedMessage, Messages responseMessage, rapidjson::Document &d,
        ...
        d["message"].SetInt(static_cast<int>(responseMessage));
        d.Accept(writer);
    ...
    void BlockchainNode::SendMessage(Messages receivedMessage, Messages responseMessage, rapidjson::Document &d,
        ...
        if(it == m_peersSockets.end())
        {
            ...
            SendMessage(receivedMessage, responseMessage, d, m_peersSockets[outgoingIpv4Address]);
        }else {
            SendMessage(receivedMessage, responseMessage, d, (*it).second);
        }


    }

    void BlockchainNode::SendMessage(Messages receivedMessage, Messages responseMessage, std::string packet,
                                     Address &outgoingAddress) {
        NS_LOG_FUNCTION(this);
        rapidjson::Document d;
        ...
        SendMessage(receivedMessage, responseMessage, d, outgoingAddress);
    }

    void BlockchainNode::InvTimeoutExpired(const std::string& blockHash) {
        NS_LOG_FUNCTION(this);
        std::string invDelimiter = "/";
        size_t invPos = blockHash.find(invDelimiter);
        ...
        && !m_blockchain.IsOrphan(height, minerId) && !ReceivedButNotValidated(blockHash)) {
            rapidjson::Document d;
            EventId timeout;
            rapidjson::Value value(static_cast<int>(Messages::INV));
            rapidjson::Value array(rapidjson::kArrayType);
            d.SetObject();
            ...
            unsigned seed = std::chrono::steady_clock::now().time_since_epoch().count();
            std::default_random_engine eng(seed);
            std::uniform_int_distribution<int> distreal(0,m_queueInv[blockHash].size());
            int index = distreal(eng);
            Address temp = m_queueInv[blockHash][0];
            m_queueInv[blockHash][0] = m_queueInv[blockHash][index];
            m_queueInv[blockHash][index] = temp;

            SendMessage(Messages::INV, Messages::GET_HEADERS, d, *(m_queueInv[blockHash].begin()));

            SendMessage(Messages::INV, Messages::GET_DATA, d, *(m_queueInv[blockHash].begin()));
    ...
    bool BlockchainNode::ReceivedButNotValidated(std::string blockHash) {
        NS_LOG_FUNCTION(this);
    ...
    void BlockchainNode::RemoveReceivedButNotvalidated(std::string blockHash) {
        NS_LOG_FUNCTION(this);
    ...
    bool BlockchainNode::OnlyHeadersReceived(const std::string& blockHash) {
