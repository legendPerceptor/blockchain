...
namespace ArcBlockChain{
    ...
    class blockchain_state_machine : public state_machine {
    public:
        blockchain_state_machine()
                : cur_value_(0)
        ...
        static ptr<buffer> enc_log(const Ledger& block) {
            // Encode from {operator, operand} to Raft log.
            ptr<buffer> ret = buffer::alloc(block.size());
            buffer_serializer bs(ret);


            bs.put_i32(block.head.seq);
            bs.put_i32(block.head.ts_amount);
            bs.put_str(block.head.prev_hash);
            bs.put_str(block.head.timestamp);

            bs.put_str(block.head.hash);
            for(auto ii : block.transactions) {
                bs.put_str(ii.A);
                bs.put_str(ii.B);
                bs.put_str(ii.amount);
            }
        ...
        static void dec_log(buffer& log, Ledger& block) {
            // Decode from Raft log to {operator, operand} pair.
            //assert(log.size() == sizeof(op_payload));

            buffer_serializer bs(log);

            block.head.seq = bs.get_i32();
            block.head.ts_amount = bs.get_i32();
            block.head.prev_hash = bs.get_str();
            block.head.timestamp = bs.get_str();

            block.head.hash = bs.get_str();
            std::string A,B, amount;
            for(int i =0; i<block.head.ts_amount;i++) {
                A = bs.get_str();
                B = bs.get_str();
                amount = bs.get_str();
                block.transactions.push_back(transaction(A,B,amount));
            }
        ...
        LedgerManger globalLedgerManager;
        LedgerManger localLedgerManager;
